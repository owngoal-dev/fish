...
: "${PROGRAM:?}"
package_id_expected="${PACKAGE_ID:-wiki.qaq.fish}"
...
for path in \
    ...
    "$prefix/usr/bin/fish_indent" \
    "$prefix/usr/bin/fish_key_reader" \
    "$prefix/usr/share/fish" \
    "$prefix/etc/fish"; do
    on_device "test -e '$path'" || { echo "error: $path is missing after install" >&2; exit 65; }
done
for program in fish fish_indent fish_key_reader; do
...
echo "==> compiled runtime paths"
on_device "$PROGRAM -c 'test \"\$__fish_data_dir\" = \"$prefix/usr/share/fish\"; and test \"\$__fish_sysconf_dir\" = \"$prefix/etc/fish\"'" || {
    echo "error: fish is using paths for the wrong bootstrap layout" >&2
    exit 65
}
...
echo "==> pipeline, redirection, and background wait"
on_device "$PROGRAM -c 'echo fish-pipe | string match -q fish-pipe; echo fish-redir >/tmp/fish-smoke; command cat /tmp/fish-smoke | string match -q fish-redir; command rm /tmp/fish-smoke; command true &; wait'" || {
    echo "error: process-control smoke test failed" >&2
    exit 65
}
