#!/usr/bin/env bash
#
# Pin this packaging repo to the newest stable fish-shell/fish-shell X.Y.Z
...
minimum_rust="$(sed -n 's/^rust-version = "\([0-9][0-9.]*\)"/\1/p' "$repository_root/build/src/Cargo.toml" | head -n1)"
[[ "$minimum_rust" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]] || {
    echo "error: upstream Cargo.toml has no workspace rust-version" >&2
    exit 65
}
python3 - "$repository_root/configuration/upstream.env" "$minimum_rust" <<'PY'
from pathlib import Path
import sys
...
if not any(line.startswith("RUST_TOOLCHAIN=") for line in lines):
    raise SystemExit(f"{path} has no RUST_TOOLCHAIN=")
path.write_text("".join(
    f"RUST_TOOLCHAIN={toolchain}\n" if line.startswith("RUST_TOOLCHAIN=") else line
    for line in lines
))
PY
echo "set RUST_TOOLCHAIN=$minimum_rust"
