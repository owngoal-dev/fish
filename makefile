...
PACKAGE_ID      ?= wiki.qaq.fish

BUILD_DIR         := $(ROOT_DIR)/build
SRC_DIR           := $(BUILD_DIR)/src
SCRATCH_DIR       = $(BUILD_DIR)/ios-$(ARCH)-$(PACKAGE_FLAVOR)
PAYLOAD_PATH_FILE = $(BUILD_DIR)/payload-path-$(PACKAGE_FLAVOR).txt
...
# it picked this boot; a rootless bootstrap lives at a fixed /var/jb. Both use
# the same source and patches, but their compiled runtime paths differ.
...
help:
	...
	@echo "  build          Cross-compile fish for iOS"
...
check:
	...
	@for input in packaging/DEBIAN/control packaging/fish.entitlements \
		packaging/release-notes.md; do \
		test -f "$(ROOT_DIR)/$$input" || { echo "error: missing $$input" >&2; exit 66; }; \
	done
	@plutil -lint "$(ROOT_DIR)/packaging/fish.entitlements"
...
build: source
	@mkdir -p "$(BUILD_DIR)"
	@"$(IOS_BUILDER)" "$(SRC_DIR)" "$(SCRATCH_DIR)" "$(PACKAGE_PREFIX)" >"$(PAYLOAD_PATH_FILE)"
